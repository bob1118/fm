...
//pgsqlDestroyFreeswitch()
func pgsqlDestroyFreeswitch(strcon string) {
	if pgdb, e := openPgsql(strcon); e != nil {
		log.Println(e.Error())
	} else {
		pgdb.MustExec(DROP_DATABASE_USER)
		closePgsql(pgdb)
	}
}
...
//pgsqlInitFreeswitchBlacklist()
func pgsqlInitFreeswitchBlacklist() {
	var err error
	var isFound bool

	if err = db.Get(&isFound, "select count(1)!=0 as isFound from pg_tables where tablename =$1", "cc_blacklist"); err != nil {
		log.Println(err.Error())
	} else {
		if !isFound {
